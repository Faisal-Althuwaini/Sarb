...
public class SimulationEngine {
	...
	public SimulationEngine(SimulatorProperties properties, DroneRegistry registry,
	...
	void seedFleet() {
		...
		int count = properties.getDroneCount();
		for (int i = 0; i < count; i++) {
			String droneId = "drone-%03d".formatted(i + 1);
			double[] point = randomPointWithinRadius(rnd, properties.getCenterLat(), properties.getCenterLon(),
					properties.getSpreadRadiusKm());
		...
		log.info("Seeded {} virtual drones around ({}, {}), radius {} km", registry.size(),
				properties.getCenterLat(), properties.getCenterLon(), properties.getSpreadRadiusKm());
	}

	@org.springframework.scheduling.annotation.Scheduled(fixedRateString = "${sarb.simulator.tick-ms}")
	void tick() {
		ThreadLocalRandom rnd = ThreadLocalRandom.current();
	...
	private void advance(Drone drone, ThreadLocalRandom rnd, double tickSeconds) {
		...
		double distanceFromCenterKm = haversineKm(properties.getCenterLat(), properties.getCenterLon(), newLat,
				newLon);
		if (distanceFromCenterKm > properties.getSpreadRadiusKm()) {
			headingDeg = normalizeDegrees(bearingDegrees(newLat, newLon, properties.getCenterLat(),
					properties.getCenterLon()) + rnd.nextDouble(-10.0, 10.0));
